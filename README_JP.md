...
[![翻訳状態](https://hosted.weblate.org/widgets/frais/-/svg-badge.svg)](https://hosted.weblate.org/engage/frais/)
...
Fraisは、Androidアプリを凍結するための自由ソフトウェアです。[GitHub Releases](https://github.com/aistra0528/Frais/releases)
...
一般的に、「凍結」は無効化を意味しますが、Fraisはアプリを一時停止したりすることもできます。
...
**Fraisで凍結されたアプリは、同じ作業モードで解凍する必要があります。**
...
```
Success: Device owner set to package com.khaled.frais
Active admin set to component {com.khaled.frais/com.khaled.frais.receiver.DeviceAdminReceiver}
```
...
このモードを使用するには、Fraisを特権システムアプリとしてインストールする必要があります。

推奨される方法は、ROMをビルドする際にFraisをインポートすることです。`Android.bp`の例：
...
- `com.khaled.frais.action.LAUNCH`
  ：ターゲットアプリを解凍して起動します。解凍されている場合は、直接起動します。`key="package"` `value="com.package.name"`

- `com.khaled.frais.action.FREEZE`
  ：ターゲットアプリを凍結します。ホームにチェックされている必要があります。`key="package"` `value="com.package.name"`

- `com.khaled.frais.action.UNFREEZE`：ターゲットアプリを解凍します。`key="package"` `value="com.package.name"`

- `com.khaled.frais.action.FREEZE_TAG`
  ：ターゲットタグ内のすべての非ホワイトリストアプリを凍結します。`key="tag"` `value="タグ名"`

- `com.khaled.frais.action.UNFREEZE_TAG`：ターゲットタグ内のすべてのアプリを解凍します。`key="tag"` `value="タグ名"`

- `com.khaled.frais.action.FREEZE_ALL`：ホームのすべてのアプリを凍結します。`extra`は必要ありません。

- `com.khaled.frais.action.UNFREEZE_ALL`：ホームのすべてのアプリを解凍します。`extra`は必要ありません。

- `com.khaled.frais.action.FREEZE_NON_WHITELISTED`：ホームのすべての非ホワイトリストアプリを凍結します。`extra`は必要ありません。

- `com.khaled.frais.action.FREEZE_AUTO`：ホームのアプリを自動的に凍結します。`extra`は必要ありません。

- `com.khaled.frais.action.LOCK`：画面をロックします。`extra`は必要ありません。

- `com.khaled.frais.action.LOCK_FREEZE`：ホームのすべてのアプリを凍結し、画面をロックします。`extra`は必要ありません。
...
Fraisをあなたの言語に翻訳するか、既存の翻訳を改善するには、[Weblate](https://hosted.weblate.org/engage/frais/)を使用してください。

[![翻訳状態](https://hosted.weblate.org/widgets/frais/-/multi-auto.svg)](https://hosted.weblate.org/engage/frais/)
